...
const state = { manifest: null, data: {}, places: [], hours: 1, day: 0, selected: null, raster: [], rasterSample: [] };
const sourceNames = { firms: "NASA FIRMS", meteo_forets: "Météo des forêts", aemet_warnings: "AEMET-waarschuwingen", aemet_danger: "AEMET-brandgevaar", cems_rapid_mapping: "CEMS Rapid Mapping", pla_alfa: "Pla Alfa (Catalonië)", galicia_irdi: "IRDI (Galicië)", meteoalarm_nl: "MeteoAlarm (NL)", meteoalarm_be: "MeteoAlarm (BE)", meteoalarm_lu: "MeteoAlarm (LU)", meteoalarm_de: "MeteoAlarm (DE)", meteoalarm_at: "MeteoAlarm (AT)", meteoalarm_ch: "MeteoAlarm (CH)", meteoalarm_it: "MeteoAlarm (IT)", meteoalarm_fr: "MeteoAlarm (FR)" };
...
const layers = {
  ...
  incidents: L.layerGroup(), warnings: L.geoJSON(null), dangerFr: L.geoJSON(null), dangerEs: L.layerGroup(), dangerRegional: L.geoJSON(null)
};
...
const { currentIncidentFeatures, dangerValidOnDay, hasPointGeometry, isClosedIncident, safeUrl } = BrandkaartHelpers;
...
const FLAME = '<svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true"><path d="M12 2c1.7 2.8-.6 4.4.4 6.8.8-.5 1.4-1.4 1.7-2.7C16.5 5 18 8.3 18 11.3a6 6 0 1 1-12 0c0-1.9.9-3.7 2.5-5.2C8.3 8 9.4 8.9 10.7 8.9 9 6.2 9.4 3.9 12 2z"/></svg>';
function detectionIcon() { return L.divIcon({ className: "hotspot-marker", html: "<span></span>", iconSize: [13,13], iconAnchor: [7,7] }); }
function incidentIcon() { return L.divIcon({ className: "incident-marker", html: FLAME, iconSize: [26,26], iconAnchor: [13,13] }); }
...
function renderVectors() {
  ...
  document.getElementById("count-dangerFr").textContent = dangers.length;
  ...
  document.getElementById("count-dangerRegional").textContent = regional.length;
}

async function renderRaster() {
  layers.dangerEs.clearLayers(); state.rasterSample = [];
  ...
  for (const entry of entries) {
    try {
      const metaPath = `data/${entry.properties.source_attrs.meta}.gz`;
      const meta = await fetchGzipJson(metaPath);
      const b = meta.bounds, imagePath = `data/${entry.properties.source_attrs.png}`;
      L.imageOverlay(imagePath, [[b.south,b.west],[b.north,b.east]], { opacity: .55, alt: `AEMET-brandgevaar ${entry.properties.area_text}`, interactive: false }).addTo(layers.dangerEs);
      ...
      state.rasterSample.push({ meta, context, entry });
    } catch (error) { console.warn("Raster kon niet worden geladen", error); }
  }
  if (!document.querySelector('[data-layer="dangerEs"]').checked) map.removeLayer(layers.dangerEs);
}
...
function sampleRaster(lat,lon) { for (const sample of state.rasterSample) { const b=sample.meta.bounds; if (lon<b.west||lon>b.east||lat<b.south||lat>b.north) continue; const x=Math.min(sample.meta.width-1,Math.max(0,Math.floor((lon-b.west)/(b.east-b.west)*sample.meta.width))); const y=Math.min(sample.meta.height-1,Math.max(0,Math.floor((b.north-lat)/(b.north-b.south)*sample.meta.height))); const pixel=sample.context.getImageData(x,y,1,1).data; if (!pixel[3]) continue; let best=null, delta=Infinity; for (const [id,def] of Object.entries(sample.meta.classes)) { const rgb=def.color.match(/[a-f\d]{2}/gi).map(v=>parseInt(v,16)); const d=Math.abs(pixel[0]-rgb[0])+Math.abs(pixel[1]-rgb[1])+Math.abs(pixel[2]-rgb[2]); if(d<delta){delta=d;best={id:Number(id),...def};} } return best; } return null; }
...
function analyzeLocation() {
  ...
  const dangerEs=sampleRaster(lat,lon), highestFr=dangerFr.sort((a,b)=>b.properties.severity_normalized-a.properties.severity_normalized)[0], highestRegional=dangerRegional.sort((a,b)=>b.properties.severity_normalized-a.properties.severity_normalized)[0];
  const dangerText=highestRegional?highestRegional.properties.severity_source:highestFr?dangerLabel(highestFr.properties.severity_normalized):dangerEs?dangerEs.label:"Geen aangesloten gevaarlaag op deze locatie";
...
function selectLocation(lat,lon,name,subtitle="Aangeklikte kaartlocatie") { state.selected={lat,lon,name,subtitle}; if(searchMarker)map.removeLayer(searchMarker); searchMarker=L.marker([lat,lon],{icon:searchIcon,keyboard:true,title:name}).addTo(map); map.flyTo([lat,lon],Math.max(map.getZoom(),8)); analyzeLocation(); }
...
async function load() {
  ...
  try {
    ...
    const jobs=[['detections',s.firms],['incidents',s.cems_rapid_mapping],['dangerFr',s.meteo_forets],['dangerEs',s.aemet_danger],['plaAlfa',s.pla_alfa],['galiciaIrdi',s.galicia_irdi]].map(async([key,source])=>{if(!source?.file)return;try{state.data[key]=await fetchGzipJson(`data/${source.file}`);}catch(error){console.warn(key,error);}});
    ...
    await Promise.all(jobs); renderDetections(); renderVectors(); await renderRaster(); renderSources();
...
document.getElementById("panel-close").onclick=()=>{document.getElementById("detail-panel").hidden=true;document.body.classList.remove("panel-open");};
